...
// Type: Intermech.MRP.Server.MRPCompositionsBrowseTask
...
internal class MRPCompositionsBrowseTask : MRPCompositionBaseTask
{
  ...
  public MRPCompositionsBrowseTask(
    string taskName,
    ...
    MRPTaskCancelEventHandler cancelHandler)
  {
    ...
    try
    {
      ...
      {
        this.Exception = (Exception) new KernelExceptionID(210, (object) "MRPCompositionsBrowseTask.Execute");
        this.State = MRPCompositionTaskState.Error;
        service2.TaskOperation = LocalizationHolder.rm.GetString("MRP.Server.TasksQueue.9");
      }
      else
      {
        try
        {
          service2.TaskOperation = LocalizationHolder.rm.GetString("MRP.Server.TasksQueue.3");
          if (!serverSession.SessionGUID.Equals(this.SessionGuid))
            serverSession = serverSession.Clone(true, "MRPCompositions.Execute") as IServerSession;
          ...
          else
          {
            ...
            MRPCompositionBrowseTask task = new MRPCompositionBrowseTask(this.Name, this.Services, (IMRPCompositionTask) this, this.rootObject, rootObjectPath, projObject, (IMRPTypedObjectRef) null, this.holder);
    ...
    finally
    {
      this.RemoveSession();
      if (!serverSession.SessionGUID.Equals(this.SessionGuid))
        serverSession.Logout("MRPCompositions.Execute");
      lock (this.syncRoot)
      {
