// Decompiled with JetBrains decompiler
// Type: ImSSP.sc_19374
// Assembly: Intermech.TechCard.Client, Version=7.0.2.1112, Culture=neutral, PublicKeyToken=null
// MVID: 2CB0EA14-C772-4814-AD48-94FC696AFE3E
// Assembly location: D:\IPS\Client\Intermech.TechCard.Client.dll
// XML documentation location: D:\IPS\Client\Intermech.TechCard.Client.xml

using Intermech.Protection;
using System;
using System.Text;

#nullable disable
namespace ImSSP;

internal static class sc_19374
{
  private static byte[] sspq = new byte[84]
  {
    (byte) 47,
    (byte) 25,
    (byte) 174,
    (byte) 120,
    (byte) 192 /*0xC0*/,
    (byte) 242,
    (byte) 67,
    (byte) 126,
    (byte) 40,
    (byte) 30,
    (byte) 16 /*0x10*/,
    (byte) 78,
    (byte) 58,
    (byte) 62,
    (byte) 159,
    (byte) 87,
    (byte) 252,
    (byte) 217,
    (byte) 251,
    (byte) 25,
    (byte) 177,
    (byte) 243,
    (byte) 152,
    (byte) 27,
    (byte) 118,
    (byte) 163,
    (byte) 22,
    (byte) 156,
    (byte) 62,
    (byte) 166,
    (byte) 251,
    (byte) 158,
    (byte) 58,
    (byte) 87,
    (byte) 55,
    (byte) 25,
    (byte) 92,
    (byte) 202,
    (byte) 102,
    (byte) 33,
    (byte) 235,
    (byte) 25,
    (byte) 201,
    (byte) 196,
    (byte) 85,
    (byte) 162,
    (byte) 231,
    (byte) 251,
    (byte) 152,
    (byte) 201,
    (byte) 200,
    (byte) 73,
    (byte) 69,
    (byte) 173,
    (byte) 162,
    (byte) 81,
    (byte) 189,
    (byte) 187,
    (byte) 43,
    (byte) 249,
    (byte) 49,
    (byte) 42,
    (byte) 42,
    (byte) 170,
    (byte) 196,
    (byte) 111,
    (byte) 211,
    (byte) 161,
    (byte) 27,
    (byte) 201,
    (byte) 165,
    (byte) 202,
    (byte) 84,
    (byte) 252,
    (byte) 187,
    (byte) 38,
    (byte) 183,
    (byte) 94,
    (byte) 27,
    (byte) 113,
    (byte) 250,
    (byte) 11,
    (byte) 237,
    (byte) 49
  };
  private static byte[] sspr = new byte[84]
  {
    (byte) 19,
    (byte) 191,
    (byte) 148,
    (byte) 95,
    (byte) 88,
    (byte) 177,
    (byte) 43,
    (byte) 225,
    (byte) 233,
    (byte) 34,
    (byte) 156,
    (byte) 244,
    (byte) 22,
    (byte) 247,
    (byte) 109,
    (byte) 253,
    (byte) 151,
    (byte) 80 /*0x50*/,
    (byte) 181,
    (byte) 52,
    (byte) 86,
    (byte) 65,
    (byte) 139,
    (byte) 130,
    (byte) 212,
    (byte) 73,
    (byte) 132,
    (byte) 85,
    (byte) 177,
    (byte) 43,
    (byte) 105,
    (byte) 184,
    (byte) 173,
    (byte) 202,
    (byte) 230,
    (byte) 8,
    (byte) 78,
    (byte) 94,
    (byte) 85,
    (byte) 22,
    (byte) 80 /*0x50*/,
    (byte) 36,
    (byte) 238,
    (byte) 15,
    (byte) 83,
    (byte) 199,
    (byte) 186,
    (byte) 150,
    (byte) 178,
    (byte) 225,
    (byte) 42,
    (byte) 209,
    (byte) 69,
    (byte) 124,
    (byte) 88,
    (byte) 58,
    (byte) 232,
    (byte) 192 /*0xC0*/,
    (byte) 120,
    (byte) 130,
    (byte) 2,
    (byte) 190,
    (byte) 121,
    (byte) 142,
    (byte) 197,
    (byte) 9,
    (byte) 217,
    (byte) 14,
    (byte) 140,
    (byte) 123,
    (byte) 234,
    (byte) 140,
    (byte) 71,
    (byte) 34,
    (byte) 53,
    (byte) 215,
    (byte) 193,
    (byte) 196,
    (byte) 91,
    (byte) 36,
    (byte) 195,
    (byte) 164,
    (byte) 198,
    (byte) 166
  };

  internal static string ssp_techcard_19375()
  {
    IProtectionKey key = ProtectionService.Key;
    if (DateTime.Now.Month == 1)
    {
      byte[] numArray1 = new byte[19];
      byte[] numArray2 = new byte[19]
      {
        (byte) 235,
        (byte) 121,
        (byte) 102,
        (byte) 51,
        (byte) 11,
        (byte) 66,
        (byte) 38,
        (byte) 20,
        (byte) 152,
        (byte) 97,
        (byte) 251,
        byte.MaxValue,
        (byte) 61,
        (byte) 142,
        (byte) 22,
        (byte) 118,
        (byte) 79,
        (byte) 124,
        (byte) 217
      };
      byte[] numArray3 = new byte[19]
      {
        (byte) 33,
        (byte) 130,
        (byte) 49,
        (byte) 68,
        (byte) 235,
        (byte) 35,
        (byte) 109,
        (byte) 111,
        (byte) 118,
        (byte) 164,
        (byte) 30,
        (byte) 220,
        (byte) 3,
        (byte) 28,
        (byte) 133,
        (byte) 230,
        (byte) 214,
        (byte) 92,
        (byte) 247
      };
      key.Query(true, 359, numArray2, numArray2);
      Array.Copy((Array) numArray2, 0, (Array) numArray1, 0, 19);
      for (int index = 0; index < 19; ++index)
        numArray1[index] ^= numArray3[index];
      byte[] numArray4 = new byte[40];
      byte[] response = new byte[40];
      Array.Copy((Array) sc_19374.sspq, 0, (Array) numArray4, 0, 40);
      key.Query(true, 359, numArray4, response);
      Array.Copy((Array) sc_19374.sspr, 0, (Array) numArray4, 0, 40);
      for (int index = 0; index < numArray4.Length; ++index)
      {
        if ((int) numArray4[index] != (int) response[index])
        {
          key.TagValue = (int) response[index];
          break;
        }
      }
      return Encoding.UTF8.GetString(numArray1);
    }
    byte[] numArray5 = new byte[19];
    byte[] numArray6 = new byte[19];
    numArray6[15] = (byte) 59;
    numArray6[1] = (byte) 226;
    numArray6[2] = (byte) 211;
    numArray6[7] = (byte) 56;
    numArray6[17] = (byte) 147;
    numArray6[5] = (byte) 63 /*0x3F*/;
    numArray6[6] = (byte) 219;
    numArray6[12] = (byte) 73;
    numArray6[8] = (byte) 176 /*0xB0*/;
    numArray6[3] = (byte) 224 /*0xE0*/;
    numArray6[10] = (byte) 220;
    numArray6[11] = (byte) 55;
    numArray6[0] = (byte) 84;
    numArray6[18] = (byte) 1;
    numArray6[14] = (byte) 112 /*0x70*/;
    numArray6[4] = (byte) 45;
    numArray6[16 /*0x10*/] = (byte) 129;
    numArray6[13] = (byte) 151;
    numArray6[9] = (byte) 37;
    byte[] numArray7 = new byte[19]
    {
      (byte) 50,
      (byte) 219,
      (byte) 175,
      (byte) 129,
      (byte) 208 /*0xD0*/,
      (byte) 106,
      (byte) 60,
      (byte) 231,
      (byte) 132,
      (byte) 231,
      (byte) 209,
      (byte) 1,
      (byte) 172,
      (byte) 97,
      (byte) 80 /*0x50*/,
      (byte) 184,
      (byte) 212,
      (byte) 156,
      (byte) 48 /*0x30*/
    };
    key.Query(true, 359, numArray6, numArray6);
    Array.Copy((Array) numArray6, 0, (Array) numArray5, 0, 19);
    for (int index = 0; index < 19; ++index)
      numArray5[index] ^= numArray7[index];
    byte[] numArray8 = new byte[44];
    byte[] response1 = new byte[44];
    Array.Copy((Array) sc_19374.sspq, 40, (Array) numArray8, 0, 44);
    key.Query(true, 359, numArray8, response1);
    Array.Copy((Array) sc_19374.sspr, 40, (Array) numArray8, 0, 44);
    for (int index = 0; index < numArray8.Length; ++index)
    {
      if ((int) numArray8[index] != (int) response1[index])
      {
        key.TagValue = (int) response1[index];
        break;
      }
    }
    return Encoding.UTF8.GetString(numArray5);
  }
}
