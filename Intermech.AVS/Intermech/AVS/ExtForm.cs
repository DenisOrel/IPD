...
/// <summary> Расширеная форма </summary>
public class ExtForm : Form, IStructualControlSupport
{
  ...
  public ExtForm() => this.Closed += new EventHandler(this.ExtForm_Closed);

  /// <summary> Конструктор позволяющий задать предка формы </summary>
  public ExtForm(Control _OwnerControl)
    : this()
  {
    this._OwnerControl = _OwnerControl;
  }
  ...
  private void InitializeComponent()
  {
    ...
    this.ClientSize = new Size(581, 196);
    this.Name = nameof (ExtForm);
  }

  /// <summary> Проверка, должно ли быть доступно редактирование </summary>
  protected virtual bool GetIsReadOnly() => false;
  ...
  /// <summary>
  /// Признак того, что признак Changed=true распостраняется "вверх" только после нажатия кнопки OK
  /// (например, данные будут изменены только после того, как это будет подтвержденно нажатием кнопки ОК)
  /// </summary>
  /// <returns> true если требудется ждать нажатия OK </returns>
  public virtual bool ChangedWaitOk() => !this._Closed;

  /// <summary>
  /// Расстояние от правого края кнопки Отмена до правой границы формы
  /// </summary>
  ...
  public void UpdateControls(bool recurce)
  {
    if (this.structualControlIntf == null)
      return;
  ...
  public bool ReadOnly
  {
    get
    {
      return this.structualControlIntf == null ? ((IStructualControlSupport) this).IsReadOnly() : this.structualControlIntf.ReadOnly;
    }
    set
    {
      if (this.structualControlIntf == null)
  ...
  /// <summary> Вызывается после загрузки формы </summary>
  ...
  /// <param name="e"></param>
  private void ExtForm_Closed(object sender, EventArgs e)
  {
    this._Closed = true;
    if (!this.Changed || this.ReadOnly || this.DialogResult != DialogResult.OK || this.structualControlIntf == null)
      return;
    this._Closed = false;
    this.structualControlIntf.Changed = false;
    this._Closed = true;
    this.structualControlIntf.Changed = true;
  }
  ...
  /// <summary> Ссылка на контрол-владелец (опционально, может быть null, зависит от вызваного конструктора) </summary>
  public Control OwnerControl => this._OwnerControl;
