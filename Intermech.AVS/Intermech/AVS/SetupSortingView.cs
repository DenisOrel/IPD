...
// Type: Intermech.AVS.SetupSortingView
...
[ViewDescriptionProvider(typeof (SetupSortingView.CustomViewDescriptionProvider))]
internal class SetupSortingView : UserControl, IView
{
  private FormSetupSorting _form;
  ...
  private string _viewName = "Сортировка";

  public SetupSortingView()
  {
    this.InitializeComponent();
    INamedImageList service = (INamedImageList) ServicesManager.GetService(typeof (INamedImageList));
    if (service == null)
      return;
    Image image = DocumentMenuHelper.LoadImageFromResurces("Intermech.Document.Model.Resources.SortSetup.bmp");
    if (image != null)
      this._imageIndex = service.Add(image, "imgAVSSortSetup");
    else
  ...
  public void Activate(IView previousView)
  {
    if (AVSPlugin.Instance.ActiveImDocumentEditorForm == null)
      return;
    ImDocument doc = AVSPlugin.Instance.ActiveImDocumentEditorForm.Document;
    if (doc == null)
      return;
    if (this._form != null)
      this._form.Parent = (Control) null;
    if (Math.Abs(doc.DBObjectID) != Math.Abs(this._selectedID))
      doc = (ImDocument) null;
    this._form = AVSPlugin.GetTemplateSetupSorting(doc, this._selectedID, this._selectedObjectTypeID, this._documentTemplateId);
