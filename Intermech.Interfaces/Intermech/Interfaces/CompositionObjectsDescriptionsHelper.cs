...
// Type: Intermech.Interfaces.CompositionObjectsDescriptionsHelper
...
namespace Intermech.Interfaces
{
    ...
    /// CompositionObject
    /// </summary>
    public static class CompositionObjectsDescriptionsHelper
    {
      /// <summary>
      ...
      private static List<object> OrderItems(List<object> items, IList<long> versions)
      {
        ...
        {
          if (!(items[index] is CompositionObject compositionObject))
            return (List<object>) null;
      ...
      public static object LoadDescription(IUserSession session, Type T, long version, long linkID)
      {
        ...
        if (session == null || version == 0L || linkID == 0L || T == (Type) null || !(Activator.CreateInstance(T) is CompositionObject instance))
          return obj;
      ...
      public static List<object> LoadDescriptions(
        ...
        int relType)
      {
        List<object> objectList = CompositionObjectsDescriptionsHelper.LoadUnsortedDescriptions(session, T, linkIDs, objectType, relType);
        objectList.Sort();
        return objectList;
      }
        ...
        int relType)
      {
        ...
        if (!(Activator.CreateInstance(T) is CompositionObject instance))
          return items;
        DataTable dataTable = (DataTable) null;
        ColumnDescriptor[] columnDescriptorArray = new ColumnDescriptor[instance.GetColumnDescriptors(Array.Empty<Tuple<object, AttributeSourceTypes>>()).Count];
        instance.GetColumnDescriptors(Array.Empty<Tuple<object, AttributeSourceTypes>>()).CopyTo(columnDescriptorArray);
        ...
        if (dataTable != null)
        {
          try
          {
            for (int index = 0; index < dataTable.Rows.Count; ++index)
            {
              ...
              if (instance.F_OBJECT_ID != 0L)
                items.Add((object) (instance.Clone() as CompositionObject));
            }
          }
          finally
          {
        ...
        return CompositionObjectsDescriptionsHelper.OrderItems(items, linkIDs);
      }
    }
}
