...
// Type: Intermech.Expressions.Functions.MinFunction
...
namespace Intermech.Expressions.Functions
{
    ...
    public class MinFunction : Function
    {
      public override object Evaluate(object[] values)
      {
        ...
        if (ExpTypeConverter.CanConvert(type, typeof (double)))
        {
          ...
          for (; index <= values.GetUpperBound(0); ++index)
          {
            ...
            {
              flag = false;
              if (Convert.ToDouble(obj) < maxValue)
                maxValue = Convert.ToDouble(obj);
            }
          }
          return !flag ? (object) maxValue : (object) DBNull.Value;
        }
      ...
      public override string Description => LocalizationHolder.rm.GetString("Interfaces_671");
    }
}
