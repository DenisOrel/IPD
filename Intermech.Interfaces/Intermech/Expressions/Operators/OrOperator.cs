...
// Type: Intermech.Expressions.Operators.OrOperator
...
namespace Intermech.Expressions.Operators
{
    ...
    {
      public override object Evaluate(object[] values) => OrOperator.staticEvaluate(values);

      internal override OperatorType GetOperatorType() => OperatorType.orOperator;
      ...
      public static object staticEvaluate(object[] values)
      {
        if (Convert.IsDBNull(values[0]))
        {
          ...
          return Convert.ToBoolean(values[1]) ? (object) true : (object) DBNull.Value;
        }
        if (!Convert.IsDBNull(values[1]))
          return (object) (bool) (Convert.ToBoolean(values[0]) ? 1 : (Convert.ToBoolean(values[1]) ? 1 : 0));
        if (!ExpTypeConverter.CanConvert(values[0], typeof (bool)))
          return (object) DBNull.Value;
        return Convert.ToBoolean(values[0]) ? (object) true : (object) DBNull.Value;
      }
