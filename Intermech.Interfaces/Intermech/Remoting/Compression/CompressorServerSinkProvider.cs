...
// Type: Intermech.Remoting.Compression.CompressorServerSinkProvider
...
{
    /// <summary>
    /// Реализует провайдер для серверного канального приемника, выполняющего сжатие сетевого трафика.
    /// </summary>
    public sealed class CompressorServerSinkProvider : IServerChannelSinkProvider
    {
      ...
      public CompressorServerSinkProvider(IDictionary properties, ICollection providerData)
      {
        if (properties == null)
          return;
      ...
      /// <summary>Создает объект.</summary>
      public CompressorServerSinkProvider()
      {
      }
      ...
      {
        return (IServerChannelSink) new CompressorServerSink(this.nextProvider.CreateSink(channel), this.enabled);
      }
