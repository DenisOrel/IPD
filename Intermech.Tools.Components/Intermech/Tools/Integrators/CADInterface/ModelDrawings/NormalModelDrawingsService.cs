...
// Type: Intermech.Tools.Integrators.CADInterface.ModelDrawings.NormalModelDrawingsService
...
/// Работа сервиса основана на знании специального расширения, имеющегося только у файлов чертежей.
/// </summary>
...
public class NormalModelDrawingsService(
  IIntegrator owner,
  ...
  params string[] modelExtensions) : AbstractModelDrawingsService(owner, drawingExtension, modelExtensions)
{
  ...
  {
    return PathUtils.IsSamePath(Path.GetExtension(fileName), this.DrawingExtension);
  }
  ...
  {
    if (PathUtils.IsSamePath(drawingFileName, modelFileName) || !PathUtils.IsSamePath(Path.GetExtension(drawingFileName), this.DrawingExtension) || !PathUtils.IsSamePath(Path.GetDirectoryName(drawingFileName), Path.GetDirectoryName(modelFileName)))
      return false;
    ...
    else if (PathUtils.IsSamePath(withoutExtension1, withoutExtension2))
      return true;
    return false;
  }
    ...
    Func<string, bool> fileExists)
  {
    ...
    {
      string firstPath = Path.ChangeExtension(modelFileName, this.DrawingExtension);
      if (!PathUtils.IsSamePath(firstPath, modelFileName) && fileExists(firstPath))
