...
// Type: Intermech.Redline.RedLineRectangleFillTool
...
[Serializable]
public class RedLineRectangleFillTool : MapTool
{
  private PointF _pointLast;
  private PointF _pointFirst;
  private MapRedRectangle _rectangle;
  private Redliner _redliner;
  private IMapRelative _relative;

  internal RedLineRectangleFillTool(Redliner redliner)
    : base(redliner.View)
  {
  ...
  public override void Stop()
  {
    if (this._rectangle != null)
    {
      ...
      this._rectangle = (MapRedRectangle) null;
    }
  ...
  {
    if (this.LastInput.Buttons != MouseButtons.Left || this._rectangle != null)
      return;
    this._pointFirst = this.LastInput.DocPoint;
    this._rectangle = new MapRedRectangle();
    this._rectangle.Pen = new Pen(this._redliner.PenColorAlpha, this._redliner.PenWidthInDrawingUnits);
    this._rectangle.Pen.DashStyle = DashStyle.Solid;
    if (this._redliner.BrushColor != Color.Empty)
      this._rectangle.Brush = (Brush) new SolidBrush(this._redliner.BrushColorAlpha);
    this._rectangle.Bounds = new RectangleF(this._pointFirst, new SizeF(0.0f, 0.0f));
    this.View.Layers.Default.Add((MapObject) this._rectangle);
  }

  /// <summary>действия когда мышь двигают</summary>
  public override void DoMouseMove()
  {
    if (this._rectangle == null || this._pointLast == this.LastInput.DocPoint)
      return;
    ...
    this._rectangle.Bounds = new RectangleF(x, y, width, height);
  }
  ...
  public override void DoMouseUp()
  {
    ...
    if (this._rectangle != null)
    {
      ...
      {
        this._rectangle.Relative = this._relative;
        this._rectangle.RelativeId = this._relative.GetId(this._rectangle.Center);
      }
      this._redliner.AddNewObject((MapObject) this._rectangle);
      this._rectangle = (MapRedRectangle) null;
      this.TransactionResult = "New RedLineRectangleFill";
