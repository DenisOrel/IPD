...
// Type: Intermech.Tools.Data.Sync.NormalAttributeSyncAction
...
public class NormalAttributeSyncAction : AttributeSyncAction
{
  private static readonly NormalAttributeSyncAction instance = new NormalAttributeSyncAction();

  /// <summary>Возвращает константный экземпляр объекта.</summary>
  public static NormalAttributeSyncAction Instance => NormalAttributeSyncAction.instance;
  ...
  {
    ValueRecord targetItem = taskData.TargetTable.Find(attribute.Key);
    if (targetItem != null)
    {
      if (sourceItem == null)
        sourceItem = taskData.SourceSyncHelper.GetOmittedValue(attribute.Key);
      if (sourceItem == null)
        return;
      this.SyncToExistingTarget(taskData, attribute, sourceItem, targetItem);
    }
    else
    {
      if (sourceItem == null)
        return;
      ValueRecord newTarget = this.SyncToNewTarget(taskData, attribute, sourceItem);
      if (newTarget == null)
        return;
      taskData.TargetTable.Add(newTarget);
    }
  }
}
