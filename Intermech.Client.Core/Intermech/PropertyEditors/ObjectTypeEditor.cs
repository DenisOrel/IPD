...
// Type: Intermech.PropertyEditors.ObjectTypeEditor
...
public class ObjectTypeEditor : UITypeEditor
{
    ...
    object value)
  {
    ...
    if (value is ObjectTypePropertyClass)
      this.selectorForm.InitSelectionAsType(new ArrayList((ICollection) new int[1]
      {
        ((ObjectTypePropertyClass) value).ObjectType
    ...
    if (this.selectorForm.ShowDialog() != DialogResult.OK || this.selectorForm.IDList.Count <= 0)
      return value;
    return (System.Type) this.selectorForm.TypeList[0] == typeof (ObjectTypesFolder) ? (object) new ObjectTypePropertyClass(-1, (string) null) : (object) new ObjectTypePropertyClass((int) this.selectorForm.IDList[0], this.selectorForm.NameList[0].ToString());
  }
