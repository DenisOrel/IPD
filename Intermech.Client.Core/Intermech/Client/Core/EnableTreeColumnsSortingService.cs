...
// Type: Intermech.Client.Core.EnableTreeColumnsSortingService
...
/// можно ли разрешать сортировку колонки в дереве "Навигатора"
/// для содержимого, которое строится на основе указанного дескриптора
/// корневого узла
/// </summary>
internal class EnableTreeColumnsSortingService : IEnableTreeColumnsSortingService
{
  /// <summary>
  /// Коллекция интерфейсов, которые позволяют выполнить анализ дескриптора
  /// </summary>
  private Dictionary<Guid, IEnableTreeColumnsSorting> _selectors = new Dictionary<Guid, IEnableTreeColumnsSorting>();
  ...
  public void Register(IEnableTreeColumnsSorting selector)
  {
    if (selector == null)
      return;
  ...
  /// </summary>
  /// <param name="selector">Класс, выполняющий проверки дескрипторов</param>
  public void Unregister(IEnableTreeColumnsSorting selector)
  {
  ...
  public bool EnableTreeColumnsSorting(IDescriptor rootDescriptor, IServiceProvider viewServices)
  {
    if (this._selectors.Count == 0)
      return true;
    int num1 = 0;
    int num2 = 0;
    foreach (KeyValuePair<Guid, IEnableTreeColumnsSorting> selector in this._selectors)
    {
      switch (selector.Value.EnableTreeColumnsSorting(rootDescriptor, viewServices))
      {
