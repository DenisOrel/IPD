...
// Type: Intermech.Navigator.LifeCycle.LifeCycleSchemeStepsPart
...
/// <summary>Список шагов, принадлежащих схеме жизненного цикла</summary>
public class LifeCycleSchemeStepsPart : IContextAware, INodeItems, INodePart, INodeQuerySupport
{
  /// <summary>Идентификатор схемы изненного цикла</summary>
  protected int id;
  /// <summary>Контейнер сервисов</summary>
  protected IServiceProvider services;
  /// <summary>Владелец данной схемы</summary>
  protected object owner;

  /// <summary>Создать экземпляр класса</summary>
  /// <param name="id">Идентификатор схемы жизненного цикла</param>
  /// <param name="services">Контейнер сервисов</param>
  public LifeCycleSchemeStepsPart(int id, IServiceProvider services)
  {
    this.id = id;
  ...
  {
    LifeCycleStepNodeID nodeID1 = nodeID as LifeCycleStepNodeID;
    IFactory service = (IFactory) ServicesManager.GetService(typeof (IFactory));
    if (nodeID1 == null)
      return (INode) null;
    return service.GetNode((INodeID) nodeID1, (object) this.id);
  }
  ...
  {
    return nodeID is LifeCycleStepNodeID lifeCycleStepNodeId ? MetaDataHelper.GetLCStepName(lifeCycleStepNodeId.id) : string.Empty;
  }
  ...
  {
    List<IMSLifeCycleStep> lcStepsList = MetaDataHelper.GetLCStepsList();
    for (int index = 0; index < lcStepsList.Count; ++index)
    {
      if (lcStepsList[index].Name == address)
        return (INodeID) new LifeCycleStepNodeID(lcStepsList[index].LCStepID);
    }
    return (INodeID) null;
  }
  ...
  {
    if (!(nodeID is LifeCycleStepNodeID lifeCycleStepNodeId))
      return (PersistentState) null;
    PersistentState persistentState = new PersistentState();
    persistentState.AddValue("id", (object) lifeCycleStepNodeId.id);
  ...
  public INodeID Deserialize(PersistentState persistNodeID)
  {
    ...
    return obj != null && obj is int id ? (INodeID) new LifeCycleStepNodeID(id) : (INodeID) null;
  }
  ...
  {
    return nodeID is LifeCycleStepNodeID lifeCycleStepNodeId && dataFormat == typeof (IDBLCStepID) ? (object) new DBLCStepID(lifeCycleStepNodeId.id, MetaDataHelper.GetLCStepName(lifeCycleStepNodeId.id)) : (object) null;
  }
  ...
  public INodeQuery GetQuery()
  {
    ...
    return (INodeQuery) new LifeCycleSchemeStepsQuery(this.id, (INodeQuerySupport) this);
  }
  ...
  public NodeColumnCollection GetDefaultColumns()
  {
    ...
    {
      (ServicesManager.GetService(typeof (IColumnSchemes)) as IColumnSchemes).CreateColumn(Intermech.Navigator.Consts.NameColumnSchemeGuid, (object) LifeCycleSchemeStepsQuery.CAPTION)
    };
  }
  ...
  {
    return column.SchemeGuid == Intermech.Navigator.Consts.NavigatorColumnSchemeGuid && (column.ID.Equals((object) LifeCycleSchemeStepsQuery.CAPTION) || column.ID.Equals((object) LifeCycleSchemeStepsQuery.ncImsScheme)) || column.SchemeGuid == Intermech.Navigator.Consts.NameColumnSchemeGuid ? column.ID : (object) null;
  }
  ...
  public List<object> GetSpecialFields()
  {
    ...
    {
      (object) LifeCycleSchemeStepsQuery.ncImsScheme
    };
  }
  ...
  {
    return !(fieldValues[adapter.GetFieldIndex((object) LifeCycleSchemeStepsQuery.ncImsScheme)] is IMSLifeCycleStep fieldValue) ? (INodeID) null : (INodeID) new LifeCycleStepNodeID(fieldValue.LCStepID);
  }
  ...
  {
    return (object) MetaDataHelper.GetLCStep(((LifeCycleStepNodeID) nodeId).id);
  }
}
