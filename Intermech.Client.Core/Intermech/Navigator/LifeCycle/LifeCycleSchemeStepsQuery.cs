...
// Type: Intermech.Navigator.LifeCycle.LifeCycleSchemeStepsQuery
...
/// <summary>Класс для получения списка шагов ЖЦ</summary>
internal sealed class LifeCycleSchemeStepsQuery : BaseNodeQuery
{
  /// <summary>Подготовка запроса к выполнению</summary>
  private INodeQuerySupport support;
  /// <summary>
  /// Идентификатор схемы ЖЦ, для которой выполняется запрос
  /// </summary>
  protected int id;
  /// <summary>Подходящие шаги ЖЦ</summary>
  protected List<IMSLifeCycleStep> items;
  /// <summary>Список найденных шагов ЖЦ</summary>
  private List<IMSLifeCycleStep> rows = new List<IMSLifeCycleStep>();
  /// <summary>
  /// Составное значение: атрибут F_LC_STEP : источник - объект (имитируем специальное поле "Шаг ЖЦ")
  /// </summary>
  ...
  {
    (object) LifeCycleSchemeStepsQuery.CAPTION,
    (object) LifeCycleSchemeStepsQuery.ncImsScheme
  };

  /// <summary>Создать экземпляр класса</summary>
  /// <param name="id">Идентификатор схемы ЖЦ, для которой создаётся запрос. -1 - возвращаются все шаги ЖЦ</param>
  /// <param name="support">Подготовка запроса к выполнению</param>
  public LifeCycleSchemeStepsQuery(int id, INodeQuerySupport support)
  {
    this.support = support;
    this.items = MetaDataHelper.GetLCStepsList();
    if (id == -1)
      return;
    for (int index = this.items.Count - 1; index >= 0; --index)
    {
      if (this.items[index].SchemaID != id)
  ...
  protected override NodeQueryResult Execute(object bookmark, int count, RecordMapping mapping)
  {
    if (mapping != null && mapping.SortFields != null && mapping.SortFields.Length != 0)
    {
      ...
      {
        flag = mapping.SortFields[index].Equals((object) LifeCycleSchemeStepsQuery.CAPTION);
        if (flag)
        {
      ...
      if (flag && nodeColumnSortOrder == NodeColumnSortOrder.Descending)
        this.items.Sort((IComparer<IMSLifeCycleStep>) new LifeCycleSchemeStepsQuery.DescStepsComparer());
    }
    ...
    return new NodeQueryResult(position2 < this.items.Count ? (object) new PositionBookmark(position2) : (object) (PositionBookmark) null, count, this.TotalRecordCount, LifeCycleSchemeStepsQuery.FieldsOrder);
  }
  ...
  protected override NodeQueryResult Execute(object[] recordIds, RecordMapping mapping)
  {
    ...
    {
      int index2 = this.items.IndexOf(recordIds[index1] as IMSLifeCycleStep);
      if (index2 >= 0)
        this.rows.Add(this.items[index2]);
    }
    return new NodeQueryResult(this.rows.Count, this.TotalRecordCount, LifeCycleSchemeStepsQuery.FieldsOrder);
  }
  ...
  /// <summary>Сравнение описаний шагов ЖЦ по убыванию</summary>
  private class DescStepsComparer : IComparer<IMSLifeCycleStep>
  {
    /// <summary>Сравнить два описания шагов ЖЦ по убыванию</summary>
    /// <param name="x">Первый шаг</param>
    /// <param name="y">Второй шаг</param>
    /// <returns>-1, 0, 1</returns>
    public int Compare(IMSLifeCycleStep x, IMSLifeCycleStep y) => -x.Name.CompareTo(y.Name);
  }
}
