...
// Type: Intermech.Document.UI.UEDocTreeViewDlg
...
/// <summary>Панель представляет дерево загруженного документа UEdit</summary>
public class UEDocTreeViewDlg : DockControl, ISkipTargetActivate
{
  private UEditDocument ueDocTree;
  ...
  /// <summary>Документ UEdit</summary>
  public UEditDocument UEDocTree
  {
    [DebuggerStepThrough] get => this.ueDocTree;
    set
    {
      if (this.ueDocTree == value)
        return;
      this.ueDocTree = value;
      this.UpdateTree();
    }
  }

  /// <summary>Конструктор</summary>
  public UEDocTreeViewDlg() => this.InitializeComponent();
  ...
  public void UpdateTree()
  {
    ...
    {
      if (this.treeList.Nodes[index].Tag is UEDocTreeListNodeWrapper tag)
        tag.RemoveNode();
      else
        this.treeList.Nodes.RemoveAt(index);
    }
    if (this.UEDocTree != null)
    {
      TreeListNode treeListNode = this.treeList.AppendNode((object) new object[1]
      {
        (object) UEDocTreeListNodeWrapper.GetDefaultCaption((CloneBase) this.UEDocTree)
      }, (TreeListNode) null);
      new UEDocTreeListNodeWrapper(treeListNode)
      {
        UEDocNode = ((CloneBase) this.UEDocTree)
      }.SynchronizeTree(true);
      TreeListNode owner = this.treeList.AppendNode((object) new object[1]
      {
        (object) LocalizationHolder.rm.GetString("Document.Model_149")
      }, treeListNode);
      new UEDocTreeListNodeWrapper(owner)
      {
        UEDocNode = ((CloneBase) this.UEDocTree.PageArea)
      }.SynchronizeTree(true);
      owner[(object) 0] = (object) LocalizationHolder.rm.GetString("Document.Model_150");
    }
  ...
  private void treeList_SelectionChanged(object sender, EventArgs e)
  {
    if (this.propertyGrid == null || this.treeList.Selection.Count <= 0)
      return;
    this.propertyGrid.SelectedObject = (object) UEDocTreeListNodeWrapper.GetDocNode(this.treeList.Selection[0]);
  }
  ...
  private void InitializeComponent()
  {
    ...
    ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (UEDocTreeViewDlg));
    ...
    this.Name = nameof (UEDocTreeViewDlg);
