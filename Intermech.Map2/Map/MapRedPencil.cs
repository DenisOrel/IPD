...
// Type: Intermech.Map.MapRedPencil
...
{
    /// <summary>ломаная рисования для RedLines с пересчёт положения объектов относительно элемента в документе</summary>
    [Serializable]
    public class MapRedPencil : MapShape, IMapRelativePosition, IMapTime, IMapToolTipText
    {
      ...
      private const int ChangedModificationTime = 1616;
      ...
      {
        MapRedPencil mapRedPencil = (MapRedPencil) base.CopyObject(env);
        if (mapRedPencil != null)
          mapRedPencil._points = (PointF[]) this._points.Clone();
        return (MapObject) mapRedPencil;
      }
      ...
      public override RectangleF Bounds
      {
        get
        {
          ...
          if (this.Relative == null || this.RelativeId == null)
            return bounds;
          SizeF baseOffsetId = this._baseOffsetId;
          SizeF sizeF1 = new SizeF(this.Relative.GetBasePoint(this.RelativeId));
          SizeF sizeF2 = sizeF1 - baseOffsetId;
          if (sizeF2 == SizeF.Empty)
            return bounds;
          bounds.Offset(sizeF2.ToPointF());
          bool skipsUndoManager = this.SkipsUndoManager;
          this.SkipsUndoManager = true;
          this._baseOffsetId = sizeF1;
          base.Bounds = bounds;
          this.SkipsUndoManager = skipsUndoManager;
          return bounds;
        }
