...
// Type: Intermech.Vault.Interfaces.XMLSettingsStorage
...
public class XMLSettingsStorage
{
  ...
  public XmlDocument document
  {
    ...
    set
    {
      if (value == null || this._document == value || value.DocumentElement == null || value.DocumentElement.Name != "VaultSettings")
        return;
    ...
    if (stream != null && stream.Length != 0L)
    {
      if (stream.Position != stream.Length - 1L)
      {
        try
        {
          ...
          if (xmlDocument.DocumentElement == null || xmlDocument.DocumentElement.Name != "VaultSettings")
            return false;
  ...
  /// <returns>Ссылка на атрибут</returns>
  public long GetAttributeAsInt32(XmlNode node, string attrName, int defValue)
  {
    if (node == null || attrName == string.Empty || node.Attributes.Count == 0)
    ...
    if (namedItem == null)
      return (long) defValue;
    int result = defValue;
    return !int.TryParse(namedItem.InnerText, out result) ? (long) defValue : (long) result;
  }
