...
// Type: Intermech.Expert.Server.ExpertObjRules
...
public class ExpertObjRules : ExpertRules
{
  public ExpertObjRules(UserSession uSession, DataTable objectsTable)
    : base(uSession, objectsTable)
  {
    this.objType = ExpertScriptType.ObjectRule;
  }
  ...
  protected override AttributeValues[] SaveData()
  {
    ...
    ExpertServer.es.SetValueToCache<AttribPair, ScriptTreeNode>(key, val, ExpertServer.es.objRules);
    ...
    service.AddEvent(ExpServerCache.cacheObjRules, (long) this.resAttrID, (long) this.resObjTypeID, this.UserSession.DataManager);
    return attributeValuesArray;
  }
  ...
  protected override void DoDelete()
  {
    base.DoDelete();
    AttribPair key = new AttribPair(this.resAttrID, this.resObjTypeID);
    ExpertServer.es.DelValueFromCache<AttribPair, ScriptTreeNode>(key, ExpertServer.es.objRules);
    ((IExpertServerSynchronizer) ServerServices.GetService(typeof (IExpertServerSynchronizer)))?.AddEvent(ExpServerCache.cacheObjRules, (long) this.resAttrID, (long) this.resObjTypeID, this.UserSession.DataManager);
  }
    ...
    CombineAttributeMode combineMode)
  {
    ...
    if (fromAttribute.AttributeID == this.resAttrID)
    {
      ...
      {
        if (this.Session.GetObject(Convert.ToInt64(dataTable.Rows[0][0])) is ExpertObjRules expertObjRules)
        {
          expertObjRules.Load();
          ScriptTreeNode root2 = XMLScripter.LoadScript(expertObjRules.Script);
          for (int index = 0; index < root1.Items.Count; ++index)
            root2.Items.Add(root1.Items[index]);
          expertObjRules.SaveScript(root2);
          expertObjRules.WriteBLOB();
          flag = true;
          this.Delete(0L);
        }
        return flag;
