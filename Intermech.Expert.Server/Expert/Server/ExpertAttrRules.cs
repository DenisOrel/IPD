...
// Type: Intermech.Expert.Server.ExpertAttrRules
...
public class ExpertAttrRules : ExpertRules
{
  public ExpertAttrRules(UserSession uSession, DataTable objectsTable)
    : base(uSession, objectsTable)
  {
    this.objType = ExpertScriptType.AttribRule;
  }
  ...
  protected override AttributeValues[] SaveData()
  {
    ...
    ExpertServer.es.SetValueToCache<AttribPair, ScriptTreeNode>(key, val, ExpertServer.es.attrRules);
    ...
    service.AddEvent(ExpServerCache.cacheAttrRules, (long) key.attribID, (long) key.objTypeID, this.UserSession.DataManager);
    return attributeValuesArray;
  }
  ...
  protected override void DoDelete()
  {
    base.DoDelete();
    AttribPair key = new AttribPair(this.resAttrID, this.resObjTypeID);
    ExpertServer.es.DelValueFromCache<AttribPair, ScriptTreeNode>(key, ExpertServer.es.attrRules);
    ((IExpertServerSynchronizer) ServerServices.GetService(typeof (IExpertServerSynchronizer)))?.AddEvent(ExpServerCache.cacheAttrRules, (long) this.resAttrID, (long) this.resObjTypeID, this.UserSession.DataManager);
  }
  ...
  {
    ScriptTreeNode root1 = XMLScripter.LoadScript(this.Script);
    bool flag = false;
    ...
    if (fromAttribute.AttributeID == this.resAttrID)
    {
      ...
      {
        if (this.Session.GetObject(Convert.ToInt64(dataTable.Rows[0][0])) is ExpertAttrRules expertAttrRules)
        {
          expertAttrRules.Load();
          ScriptTreeNode root2 = XMLScripter.LoadScript(expertAttrRules.Script);
          for (int index = 0; index < root1.Items.Count; ++index)
            root2.Items.Add(root1.Items[index]);
          expertAttrRules.SaveScript(root2);
          expertAttrRules.WriteBLOB();
          flag = true;
          this.Delete(0L);
        }
        return flag;
