...
// Type: Intermech.Interfaces.MRP.MRPCompositionObjectHelper
...
/// MRPCompositionObject
/// </summary>
public static class MRPCompositionObjectHelper
{
  /// <summary>
  ...
  private static List<object> OrderItems(List<object> items, IList<long> versions)
  {
    ...
    {
      if (!(items[index] is MRPCompositionObject compositionObject))
        return (List<object>) null;
  ...
  public static object LoadDescription(IUserSession session, Type T, long version, long linkID)
  {
    ...
    if (session == null || version == 0L || linkID == 0L || T == (Type) null || !(Activator.CreateInstance(T) is MRPCompositionObject instance))
      return obj;
  ...
  public static List<object> LoadDescriptions(
    ...
    int relType)
  {
    List<object> objectList = MRPCompositionObjectHelper.LoadUnsortedDescriptions(session, T, linkIDs, objectType, relType);
    objectList.Sort();
    return objectList;
  }
    ...
    int relType)
  {
    ...
    if (!(Activator.CreateInstance(T) is MRPCompositionObject instance))
      return items;
    ...
    if (dataTable != null)
    {
      try
      {
        for (int index = 0; index < dataTable.Rows.Count; ++index)
        {
          ...
          if (instance.F_OBJECT_ID != 0L)
            items.Add((object) (instance.Clone() as MRPCompositionObject));
        }
      }
      finally
      {
    ...
    return MRPCompositionObjectHelper.OrderItems(items, linkIDs);
  }
}
