// Decompiled with JetBrains decompiler
// Type: Intermech.XmlExchange.ConfigEditor.TimeZoneEditor
// Assembly: Intermech.XmlExchange.ConfigEditor, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: D148B79A-64FF-4CB8-A129-56A9018E56E2
// Assembly location: D:\IPS\Client\Intermech.XmlExchange.ConfigEditor.dll

using System;
using System.Collections.ObjectModel;
using System.ComponentModel;
using System.Drawing.Design;
using System.Windows.Forms;
using System.Windows.Forms.Design;

#nullable disable
namespace Intermech.XmlExchange.ConfigEditor;

internal class TimeZoneEditor : UITypeEditor
{
  private ReadOnlyCollection<TimeZoneInfo> _timeZones = TimeZoneInfo.GetSystemTimeZones();

  public override object EditValue(
    ITypeDescriptorContext context,
    System.IServiceProvider provider,
    object value)
  {
    IWindowsFormsEditorService formsEditorService = (IWindowsFormsEditorService) null;
    if (provider != null)
      formsEditorService = provider.GetService(typeof (IWindowsFormsEditorService)) as IWindowsFormsEditorService;
    if (formsEditorService != null)
    {
      ListBox listBox = new ListBox();
      listBox.BorderStyle = BorderStyle.None;
      listBox.IntegralHeight = true;
      listBox.Click += new EventHandler(this.listBox_Click);
      listBox.Tag = (object) formsEditorService;
      string empty = string.Empty;
      if (value != null)
        empty = value.ToString();
      foreach (TimeZoneInfo timeZone in this._timeZones)
      {
        listBox.Items.Add((object) timeZone.DisplayName);
        if (empty == timeZone.Id)
          listBox.SelectedItem = (object) timeZone.DisplayName;
      }
      formsEditorService.DropDownControl((Control) listBox);
      if (listBox.SelectedItem != null)
      {
        foreach (TimeZoneInfo timeZone in this._timeZones)
        {
          if (listBox.SelectedItem.ToString() == timeZone.DisplayName)
            return (object) timeZone.Id;
        }
      }
    }
    return value;
  }

  private void listBox_Click(object sender, EventArgs e)
  {
    if (!(sender is ListBox listBox) || listBox.SelectedItem == null || !(listBox.Tag is IWindowsFormsEditorService tag))
      return;
    tag.CloseDropDown();
  }

  public override UITypeEditorEditStyle GetEditStyle(ITypeDescriptorContext context)
  {
    return UITypeEditorEditStyle.DropDown;
  }
}
