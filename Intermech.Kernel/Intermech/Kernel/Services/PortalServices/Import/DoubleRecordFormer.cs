...
// Type: Intermech.Kernel.Services.PortalServices.Import.DoubleRecordFormer
...
internal class DoubleRecordFormer(
  ...
  string path) : RecordFormer(session, eventHelper, links, measures, path)
{
    ...
    AttributeRecord record)
  {
    ...
    if (rec.DoubleValue != double.MinValue)
      result = rec.DoubleValue;
    else if (rec.IntegerValue != long.MinValue)
      result = Convert.ToDouble(rec.IntegerValue);
    else if (rec.StringValue != string.Empty)
      double.TryParse(rec.StringValue, out result);
    if (result == double.MinValue)
    {
      ...
      record.DoubleValue = (object) result;
  }
}
