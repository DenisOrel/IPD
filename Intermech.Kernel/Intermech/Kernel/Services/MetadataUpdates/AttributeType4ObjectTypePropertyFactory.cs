...
// Type: Intermech.Kernel.Services.MetadataUpdates.AttributeType4ObjectTypePropertyFactory
...
internal sealed class AttributeType4ObjectTypePropertyFactory(Guid attributeGuid, int attributeID) : 
  AttributeType4TypePropertyFactory(attributeGuid, attributeID)
{
  public Attribute4ObjectTypeProperties GetAttributeTypeProperties(IUserSession session)
  {
    ...
    return new Attribute4ObjectTypeProperties()
    {
      ...
      LevelID = this.GetPropertyValue<int>("F_LEVEL_ID", attributeType.LevelID),
      ...
      InheritMode = this.GetPropertyValue<InheritModes>("F_PUBLIC", InheritModes.Public),
      RequiredMode = this.GetPropertyValue<RequiredModes>("F_REQUIRED", RequiredModes.Manual),
      UniqueValueMode = this.GetPropertyValue<UniqueValueModes>("F_UNIQUE", attributeType.UniqueMode),
  ...
  protected override IPropertyNode GetPropertyNode(
    ...
    string nodeID)
  {
    return !(nodeID == "F_REQUIRED") ? base.GetPropertyNode(session, node, nodeID) : (IPropertyNode) new EnumNode<RequiredModes>(session, node, nodeID);
  }
}
