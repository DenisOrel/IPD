...
// Type: Intermech.UI.Wpf.ViewModels.ContextMenuUICommand`1
...
public class ContextMenuUICommand<T> : ICommand
{
  private string text;
  ...
  public ContextMenuUICommand(string text, Action<T> execute, Predicate<T> canExecute = null)
  {
    if (text == null)
  ...
  {
    return this.canExecuteFunc == null || this.canExecuteFunc((T) parameter);
  }

  /// <summary>Выполняет команду.</summary>
  /// <param name="parameter">Параметр выполнения команды</param>
  void ICommand.Execute(object parameter) => this.executeAction((T) parameter);
