...
// Type: Intermech.Tools.Setup.LaunchActionDataPage
...
internal sealed class LaunchActionDataPage : UserControl
{
  private LaunchActionInfo selectedAction;
  ...
  public LaunchActionDataPage() => this.InitializeComponent();

  public void InitializePage(LaunchActionInfo actionInfo, bool readOnly)
  {
    if (actionInfo == null)
      throw new ArgumentNullException();
    if (this.PageInitialized)
      this.CloseDataEditor();
    this.OpenDataEditor(actionInfo, readOnly);
  }
  ...
  public LaunchActionInfo SelectedAction => this.selectedAction;
  ...
  private void OpenDataEditor(LaunchActionInfo actionInfo, bool readOnly)
  {
    XmlDocument data = new XmlDocument();
    using (SessionKeeper sessionKeeper = new SessionKeeper())
    {
      ILaunchActionServer service = ServiceUtils.GetService<ILaunchActionServer>((object) sessionKeeper.Session, false);
      data.LoadXml(service.GetActionData(actionInfo.ActionId));
    }
    this.dataEditor = ClientContext.LaunchActions.GetHandler(actionInfo.HandlerId, false)?.CreateSettingsEditor();
    ...
    this.selectedAction = actionInfo;
  ...
  private void CloseDataEditor()
  {
    ...
    this.selectedAction = (LaunchActionInfo) null;
    this.dataChanged = false;
  }
  ...
  private void ApplyChanges()
  {
    ...
    using (SessionKeeper sessionKeeper = new SessionKeeper())
    {
      ...
      service.SetActionData(this.selectedAction.ActionId, data.OuterXml);
      this.selectedAction = service.GetActionInfo(this.selectedAction.ActionId);
    }
  ...
  {
    ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (LaunchActionDataPage));
    ...
    this.Name = nameof (LaunchActionDataPage);
