...
// Type: Intermech.Expert.OpParmGlobForType
...
public class OpParmGlobForType : OpParm, IDataAttrs, ILinkObjTypes
{
  public long excerptID;
  public string excerptName = "";
  public IspMode ispWork;
  public List<string> forObjTypeGUIDs;
  public List<string> forObjTypeTexts;
  public List<bool> forObjTypeOnly;
  ...
  public OpParmGlobForType()
  {
  }

  public OpParmGlobForType(ref OpParmData opData)
    : base(ref opData)
  {
  ...
  public override void SetData(ref OpParmData opData)
  {
    ...
    if (opData.forGUIDs.Count == 0)
    {
      this.forObjTypeGUIDs = (List<string>) null;
    }
    else
    {
      if (this.forObjTypeGUIDs == null)
        this.forObjTypeGUIDs = new List<string>();
      else
        this.forObjTypeGUIDs.Clear();
      for (int index = 0; index < opData.forGUIDs.Count; ++index)
        this.forObjTypeGUIDs.Add(opData.forGUIDs[index]);
    }
    if (opData.forTexts.Count == 0)
    {
      this.forObjTypeTexts = (List<string>) null;
    }
    else
    {
      if (this.forObjTypeTexts == null)
        this.forObjTypeTexts = new List<string>();
      else
        this.forObjTypeTexts.Clear();
      for (int index = 0; index < opData.forTexts.Count; ++index)
        this.forObjTypeTexts.Add(opData.forTexts[index]);
    }
    if (opData.forOT_Only.Count == 0)
    {
      this.forObjTypeOnly = (List<bool>) null;
    }
    else
    {
      if (this.forObjTypeOnly == null)
        this.forObjTypeOnly = new List<bool>();
      else
        this.forObjTypeOnly.Clear();
      for (int index = 0; index < opData.forOT_Only.Count; ++index)
        this.forObjTypeOnly.Add(opData.forOT_Only[index]);
    }
  ...
  public override void FillOpParmData(ref OpParmData opData)
  {
    ...
    if (this.forObjTypeGUIDs != null)
    {
      for (int index = 0; index < this.forObjTypeGUIDs.Count; ++index)
      {
        opData.forGUIDs.Add(this.forObjTypeGUIDs[index]);
        opData.forTexts.Add(this.forObjTypeTexts[index]);
        opData.forOT_Only.Add(this.forObjTypeOnly[index]);
      }
    }
  ...
  public override void WriteToXML(ref XmlTextWriter writer)
  {
    ...
    if (this.forObjTypeGUIDs != null && this.forObjTypeGUIDs.Count > 0)
    {
      writer.WriteStartElement("for-objtypes");
      writer.WriteAttributeString("num", Convert.ToString(this.forObjTypeGUIDs.Count));
      for (int index = 0; index < this.forObjTypeGUIDs.Count; ++index)
        writer.WriteElementString("GUID", Convert.ToString(this.forObjTypeGUIDs[index]));
      for (int index = 0; index < this.forObjTypeOnly.Count; ++index)
        writer.WriteElementString("ONLY", this.forObjTypeOnly[index] ? "Y" : "N");
      writer.WriteEndElement();
    }
  ...
  public override void LoadFromXML(XmlNode node, int opTag)
  {
    ...
    foreach (XmlNode childNode1 in node.ChildNodes)
    {
      ...
      else
      {
        ...
        if (childNode1.NodeType == XmlNodeType.Element && childNode1.Name == "ConfigOptions")
        ...
        else
        {
          ...
          if (childNode1.NodeType == XmlNodeType.Element && childNode1.Name == "for-objtypes" && childNode1.HasChildNodes)
          {
            if (this.forObjTypeGUIDs == null)
              this.forObjTypeGUIDs = new List<string>();
            if (this.forObjTypeTexts == null)
            ...
            {
              if (childNode5.Name == "GUID" && childNode5.HasChildNodes)
              {
                string innerText = childNode5.ChildNodes[0].InnerText;
                if (GuidHelper.IsGuid(innerText))
                {
                  IMSObjectType objectType = MetaDataHelper.GetObjectType(new Guid(innerText));
                  if (objectType != null)
                  {
                    this.forObjTypeGUIDs.Add(innerText);
                    this.forObjTypeTexts.Add(objectType.Text);
                  }
                }
              }
              if (childNode5.Name == "ONLY" && childNode5.HasChildNodes)
                this.forObjTypeOnly.Add(childNode5.ChildNodes[0].InnerText == "Y");
            }
          }
        }
      }
  ...
  public override bool CollectExpObjInfo(ExpObjInfo eoi, IUserSession ius)
  {
    ...
    if (this.forObjTypeGUIDs != null)
    {
      foreach (string forObjTypeGuiD in this.forObjTypeGUIDs)
        flag = flag && eoi.AddObjType(forObjTypeGuiD);
    }
    if (this.linkTypeIDs != null)
    {
      foreach (int linkTypeId in this.linkTypeIDs)
    ...
    IUserSession session)
  {
    bool flag = false;
    if (this.afterFilter != null)
      flag = this.afterFilter.PerformAttrChange(fromAttribute, toAttribute);
  ...
  public string GetObjTypesStr()
  {
    StringBuilder stringBuilder = new StringBuilder();
    if (this.forObjTypeTexts != null)
    {
      for (int index = 0; index < this.forObjTypeTexts.Count; ++index)
      {
        stringBuilder.Append(this.forObjTypeTexts[index]);
        if (index < this.forObjTypeTexts.Count - 1)
