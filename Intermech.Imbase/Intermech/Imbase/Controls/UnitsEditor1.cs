...
// Type: Intermech.Imbase.Controls.UnitsEditor1
...
internal class UnitsEditor1 : DropDownEditor
{
    ...
    object value)
  {
    ...
    if (context.Instance is RestructuringPropGridDescriptor instance)
    {
      long num1 = -1;
      List<long> longList = (List<long>) null;
      using (SessionKeeper sessionKeeper = new SessionKeeper())
      {
        IDBAttributeType attributeType = sessionKeeper.Session.GetAttributeType(instance.Settings.AttributeID);
      ...
      if (value != null && value != DBNull.Value && !string.IsNullOrEmpty(value.ToString()))
      {
        ...
        if (num2 != -1L)
        {
          foreach (object obj2 in measureDescriptorList)
          {
            if ((obj2 as MeasureDescriptor).MeasureID == num2)
            {
              listBox.SelectedItem = obj2;
              break;
            }
          }
        }
      ...
      listBox.Click -= new EventHandler(this.ListBoxClick);
      if (listBox.SelectedItem != null && !(listBox.SelectedItem as MeasureDescriptor).Empty)
      {
        using (SessionKeeper sessionKeeper = new SessionKeeper())
        {
          QuickObjectInfo objectInfo = sessionKeeper.Session.GetObjectInfo((listBox.SelectedItem as MeasureDescriptor).MeasureID);
          obj1 = !objectInfo.Empty ? (object) objectInfo.VersionGuid.ToString() : (object) Guid.Empty.ToString();
        }
      }
      else
        obj1 = (object) Guid.Empty.ToString();
    }
    return obj1;
  }
}
